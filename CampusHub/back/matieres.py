...
def creer_matiere(matiere: MatiereCreation, usr_mail: str = Depends(get_current_user)):
    with get_connection() as conn:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("INSERT INTO t_matiere_mat (mat_nom, mat_couleur, mat_note, usr_mail) VALUES (%s, %s, %s, %s);", (matiere.mat_nom, matiere.mat_couleur, matiere.mat_note, usr_mail))
            conn.commit()
...
def lire_matieres(usr_mail: str = Depends(get_current_user)):
    with get_connection() as conn:
        ...
        with conn.cursor() as cursor:
            cursor.execute("SELECT mat_id, mat_nom, mat_couleur, mat_note FROM t_matiere_mat WHERE usr_mail = %s ORDER BY mat_nom;", (usr_mail,))
...
def modifier_matiere(mat_id: int, matiere: MatiereModification, usr_mail: str = Depends(get_current_user)):
    with get_connection() as conn:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("UPDATE t_matiere_mat SET mat_nom = %s, mat_couleur = %s, mat_note = %s WHERE mat_id = %s AND usr_mail = %s;", (matiere.mat_nom, matiere.mat_couleur, matiere.mat_note, mat_id, usr_mail))
...
def lire_taches_matiere(mat_id: int, usr_mail: str = Depends(get_current_user)):
    with get_connection() as conn:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("SELECT t.tac_id, t.tac_nom, t.tac_duree, t.tac_status, t.tac_date_limite, t.tac_priorite, t.tac_note FROM t_lien_matiere_tache_lmt l JOIN t_tache_tac t USING(tac_id) WHERE l.mat_id = %s AND t.usr_mail = %s ORDER BY t.tac_nom;", (mat_id, usr_mail))
            resultats = cursor.fetchall()
