...
CREATE TABLE t_tache_tac (
    ...
    tac_date_limite DATE NULL,
    tac_priorite CHAR(1) NOT NULL DEFAULT 'M' CHECK (tac_priorite IN ('B', 'M', 'H')),
    tac_note TEXT,
    usr_mail VARCHAR(255) NOT NULL,
...
--table pour les événements fixes dans un calendrier
CREATE TABLE t_evenement_evt (
    evt_id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    evt_couleur CHAR(7) DEFAULT '#3B82F6',
    usr_mail VARCHAR(254) NOT NULL,
    CONSTRAINT fk_evt_usr FOREIGN KEY (usr_mail) REFERENCES t_utilisateur_usr(usr_mail) ON DELETE CASCADE,
    CONSTRAINT chk_evt_type CHECK (evt_type IN ('COURS', 'TD', 'TP', 'EXAMEN', 'TRAVAIL', 'PERSONNEL', 'SPORT', 'AUTRE')),
    CONSTRAINT chk_evt_heures CHECK (evt_heure_fin > evt_heure_debut)
);