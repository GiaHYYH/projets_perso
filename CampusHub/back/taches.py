...
def creer_tache(tache: TacheCreation, usr_mail: str = Depends(get_current_user)):
    with get_connection() as conn:
        ...
        with conn.cursor() as cursor:
            cursor.execute("INSERT INTO t_tache_tac (tac_nom, tac_duree, tac_status, tac_date_limite, tac_priorite, tac_note, usr_mail) VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING tac_id;", (tache.tac_nom, tache.tac_duree, "A", tache.tac_date_limite, tache.tac_priorite, tache.tac_note, usr_mail))
...
def lire_taches(usr_mail: str = Depends(get_current_user)):
    with get_connection() as conn:
        ...
        with conn.cursor() as cursor:
            cursor.execute("SELECT tac_id, tac_nom, tac_duree, tac_status, tac_date_limite, tac_priorite, tac_note FROM t_tache_tac WHERE usr_mail = %s;", (usr_mail,))
            resultats = cursor.fetchall()

            liste = []

            for resultat in resultats:
                liste.append({"tac_id": resultat[0], "tac_nom": resultat[1], "tac_duree": resultat[2], "tac_status": resultat[3], "tac_date_limite": resultat[4], "tac_priorite": resultat[5], "tac_note": resultat[6]})
...
def modifier_tache(tac_id: int, modification: TacheModification, usr_mail: str = Depends(get_current_user)):
    with get_connection() as conn:
        ...
        with conn.cursor() as cursor:
            cursor.execute("UPDATE t_tache_tac SET tac_nom = %s, tac_duree = %s, tac_date_limite = %s, tac_priorite = %s, tac_note = %s WHERE tac_id = %s AND usr_mail = %s;", (modification.tac_nom, modification.tac_duree, modification.tac_date_limite, modification.tac_priorite, modification.tac_note, tac_id, usr_mail))
...
def lire_matieres_tache(tac_id: int, usr_mail: str = Depends(get_current_user)):
    with get_connection() as conn:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("SELECT m.mat_id, m.mat_nom, m.mat_couleur, m.mat_note FROM t_lien_matiere_tache_lmt l JOIN t_matiere_mat m USING(mat_id) WHERE l.tac_id = %s AND m.usr_mail = %s;", (tac_id, usr_mail))
            resultats = cursor.fetchall()
            
            liste = []
            
            for resultat in resultats:
                liste.append({"mat_id": resultat[0], "mat_nom": resultat[1], "mat_couleur": resultat[2], "mat_note": resultat[3]})
